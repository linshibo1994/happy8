...
class Happy8Analyzer:
    ...
    def load_data(self, periods: Optional[int] = None) -> pd.DataFrame:
        ...
        if self.historical_data is None:
            ...
            if not self.historical_data.empty and 'issue' in self.historical_data.columns:
                self.historical_data = self.historical_data.copy()
                self.historical_data['issue'] = self.historical_data['issue'].astype(str)
        ...
        if periods and periods > 0:
            return self.historical_data.head(periods).copy()  # 最新期在前
        return self.historical_data.copy()

    def _select_prediction_training_data(
        self,
        target_issue: str,
        periods: int,
    ) -> pd.DataFrame:
        """按预测目标选择训练窗口，返回最新期在前的数据。"""
        data = self.load_data()
        if data.empty:
            return data

        window = data.copy()
        if 'issue' in window.columns:
            window['issue'] = window['issue'].astype(str)
            window = window[window['issue'] < str(target_issue)]
            window = window.sort_values('issue', ascending=False).reset_index(drop=True)

        if periods and periods > 0:
            window = window.head(periods)
    ...
    def predict(self,
        ...
        # 加载完整历史后按目标期裁剪，避免历史验证时泄露目标期及之后的数据
        data = self._select_prediction_training_data(
            target_issue=target_issue,
            periods=periods,
        )
